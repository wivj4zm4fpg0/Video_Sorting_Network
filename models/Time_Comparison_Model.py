...
class TimeComparison(nn.Module):
    def __init__(self, bidirectional: bool = True, pretrained: bool = True):
        ...
        if bidirectional:
            ...
            self.lstm = nn.LSTM(cnn_last_dim, int(lstm_dim / 2), bidirectional=True, num_layers=2,
                                batch_first=batch_first)
        else:
            self.lstm_forward = self.normal_forward
            self.lstm = nn.LSTM(cnn_last_dim, lstm_dim, bidirectional=False, num_layers=2, batch_first=batch_first)
    ...
    def bidirectional_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x0 = torch.stack([torch.flatten(self.cnn(x[i]), 1) for i in range(0, seq_half)])
        x0 = torch.mean(self.lstm(x0)[0], 0)
        x1 = torch.stack([torch.flatten(self.cnn(x[i]), 1) for i in range(seq_half, seq_len)])
